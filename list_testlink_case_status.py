...
def send_content(result):
    ...
    if vteam_total != 0:
        content = f"<font color=\"warning\">testlink还有{vteam_total}个用例Status不是Final,请尽快用例评审!!!</font> \n" \
                  f"<font color=\"info\">testlink可以搜索 Status!=Final</font> \n\n"
        for ret in result:
            if ret['total'] != 0:
                suites = []
        ...
        content = "恭喜恭喜，testlink所有用例都通过评审啦"
    print(content)
    if content != "":
