...
def send_content_error_output(results):
    contents = ""
    if results:
        contents = "测试任务jira关闭时，Output URL为空，请检查！！！\n"
...
def send_content_no_testplan(results):
    contents = ""
    if results:
        contents = "测试任务jira关闭时，testlink中还没有以jira号开头的测试计划 或者测试计划中没有添加测试用例，请检查！！！\n"
    for result in results:
        contents = contents + "@{} {}:{} \n".format(result['name'], result['id'], result['summary'])
    print(contents)
    if contents != "":
...
def send_content_no_logtime(results):
    contents = ""
    if results:
        contents = "测试任务jira关闭时，没有logtime，请检查！！！\n"
    for result in results:
        contents = contents + "@{} {}:{} \n".format(result['name'], result['id'], result['summary'])
    print(contents)
    if contents != "":
...
jql = 'project = ACP AND (summary ~ 测试任务  OR summary ~ 功能测试执行) AND assignee in (membersOf(test))  AND status = Resolved  AND resolved >= 2023-01-01 ORDER BY assignee ASC,resolutiondate DESC'
