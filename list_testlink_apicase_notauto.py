    ...
    for assign in suite_assign:
        for suite_tmp in assign['suites']:
            ...
            for case in cases:
                ...
                if importance == '3' and execution_type == '1':
...
def send_content(result):
    ...
    if vteam_total != 0:
        content = f"<font color=\"warning\">testlink还有{vteam_total}个用例 优先级是High但没有加API自动化,请检查!!!如果不能写自动化，请找史京南确认后加上keyword:api_no_automation</font> \n" \
                  f"<font color=\"info\">testlink可以搜索 Keyword包含api 不包含api_no_automation;Importance=High;Execution Type=Manual </font>\n\n"
        for ret in result:
            if ret['total'] != 0:
                suites = []
        ...
        content = "恭喜恭喜，testlink High的用例都加完API自动化啦"
    print(content)
    if content != "":
